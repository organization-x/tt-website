<script lang="ts">
	...
	// If the user has autoplay disabled, go to the end of the video so the graphic
	// still makes sense
	$: if (duration && video.currentTime === 0)
		video.play().catch(() => (video.currentTime = duration));
</script>
...
>
	...
	>
		<video bind:this={video} bind:duration muted playsinline>
