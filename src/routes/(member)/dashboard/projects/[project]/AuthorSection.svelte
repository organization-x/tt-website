<script lang="ts">
	...
	const onClick = (user: Pick<User, "id" | "name" | "url">) => {
		// If the user is already in the authors, ignore
		if (authors.find((author) => author.user.id === user.id)) return;
	...
	const onDelete = (id: string, cant: boolean) =>
		!cant && (authors = authors.filter((author) => author.user.id !== id));
</script>
			...
			>
				{#each authors as author (author.user.id)}
					...
					<AuthorEditor
						...
						{isOwner}
						{cantRemove}
						on:delete={() => onDelete(author.user.id, cantRemove)}
					/>
				{/each}
			</Scrollable>
