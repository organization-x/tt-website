...
<div
	on:outroend
	out:slide|local={{ duration: deleting ? 500 : 0 }}
...
>
	<div class="relative">
		...
		>
			{#each project.authors as author, i (author.user.id)}
	...
	<div class="flex flex-col py-4 px-3 min-h-116 lg:px-4">
		...
		<div class="flex gap-3 mt-auto px-1 pt-4 items-center">
			{#each project.skills as icon (icon)}
				<svelte:component
					this={getIcon(icon)}
			...
			<div class="flex gap-5 items-center ml-auto md:hidden">
				...
				<button on:click={() => (open = !open)} tabindex={$tabindex}>
